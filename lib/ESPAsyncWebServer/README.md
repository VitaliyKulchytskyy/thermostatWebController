...
- [ESPAsyncWebServer](#espasyncwebserver)
  ...
  - [Principles of operation](#principles-of-operation)
    - [The Async Web g_webServer](#the-async-web-server)
  ...
  - [Async Event Source Plugin](#async-event-source-plugin)
    - [Setup Event Source on the g_webServer](#setup-event-source-on-the-server)
  ...
  - [Setting up the g_webServer](#setting-up-the-server)
...
- When you send the response, you are immediately ready to handle other connections
  while the g_webServer is taking care of sending the response in the background
- Speed is OMG
- Easy to use API, HTTP Basic and Digest MD5 Authentication (default), ChunkedResponse
- Easily extendible to handle any type of content
- Supports Continue 100
- Async WebSocket plugin offering different locations without extra servers or ports
- Async EventSource (Server-Sent Events) plugin to send g_events to the browser
- URL Rewrite plugin for conditional and permanent url rewrites
- ServeStatic plugin that supports cache, Last-Modified, default index and more
- Simple template processing engine to handle templates

## Important things to remember
- This is fully asynchronous g_webServer and as such does not run on the loop thread.
- You can not use yield or delay or any function that uses them inside the callbacks
- The g_webServer is smart enough to know when to close the connection and free resources
- You can not send more than one response to a single request

## Principles of operation

### The Async Web g_webServer
...
### Request Life Cycle
- TCP connection is received by the g_webServer
- The connection is wrapped inside ```Request``` object
- When the request head is received (type, url, get params, http version and host),
  the g_webServer goes through all ```Rewrites``` (in the order they were added) to rewrite the url and inject query parameters,
...
- All ```Rewrites``` are evaluated on the request in the order they have been added to the g_webServer.
...
- One ```Handler``` instance can be attached to any request and lives together with the g_webServer
...
- ```Handlers``` are evaluated in the order they are attached to the g_webServer. The ```canHandle``` is called only
...
g_webServer.addHandler(handler);
```
...
In addition to serving files from SPIFFS as described above, the g_webServer provide a dedicated handler that optimize the
performance of serving files from SPIFFS - ```AsyncStaticWebHandler```. Use ```g_webServer.serveStatic()``` function to
initialize and add a new instance of ```AsyncStaticWebHandler``` to the g_webServer.
The Handler will not handle the request if the file does not exists, e.g. the g_webServer will continue to look for another
...
g_webServer.serveStatic("/page.htm", SPIFFS, "/www/page.htm");
```
...
// Request to the root or none existing files will try to g_webServer the defualt
// file name "index.htm" if exists
g_webServer.serveStatic("/", SPIFFS, "/www/");

// Server with different default file
g_webServer.serveStatic("/", SPIFFS, "/www/").setDefaultFile("default.html");
```
...
It is possible to specify Cache-Control header value to reduce the number of calls to the g_webServer once the client loaded
...
g_webServer.serveStatic("/", SPIFFS, "/www/").setCacheControl("max-age=600");

//*** Change Cache-Control after g_webServer setup ***

// During setup - keep a pointer to the handler
AsyncStaticWebHandler* handler = &g_webServer.serveStatic("/", SPIFFS, "/www/").setCacheControl("max-age=600");
...
It is possible to specify Date-Modified header to enable the g_webServer to return Not-Modified (304) response for requests
...
g_webServer.serveStatic("/", SPIFFS, "/www/").setLastModified("Mon, 20 Jun 2016 14:00:00 GMT");
...
AsyncStaticWebHandler* handler = &g_webServer.serveStatic("/", SPIFFS, "/www/");
...
g_webServer.serveStatic("/", SPIFFS, "/www/").setTemplateProcessor(processor);
```
...
```cpp
  g_webServer.addRewrite( new OneParamRewrite("/radio/{frequence}", "/radio?f={frequence}") );
```
...
```cpp
g_webServer.serveStatic("/", SPIFFS, "/www/").setFilter(ON_STA_FILTER);
g_webServer.serveStatic("/", SPIFFS, "/ap/").setFilter(ON_AP_FILTER);
```

### Rewrite to different index on AP
```cpp
// Serve the file "/www/index-ap.htm" in AP, and the file "/www/index.htm" on STA
g_webServer.rewrite("/", "index.htm");
g_webServer.rewrite("/index.htm", "index-ap.htm").setFilter(ON_AP_FILTER);
g_webServer.serveStatic("/", SPIFFS, "/www/");
```
...
// Server setup: g_webServer files in "/host1/" to requests for "host1", and files in "/www/" otherwise.
g_webServer.serveStatic("/", SPIFFS, "/host1/").setFilter(filterOnHost1);
g_webServer.serveStatic("/", SPIFFS, "/www/");
```
...
The g_webServer includes a web socket plugin which lets you define different WebSocket locations to connect to
without starting another listening service or using different port

### Async WebSocket Event
```cpp

void onEvent(AsyncWebSocket * g_webServer, AsyncWebSocketClient * client, AwsEventType type, void * arg, uint8_t *data, size_t len){
  if(type == WS_EVT_CONNECT){
    //client connected
    os_printf("ws[%s][%u] connect\n", g_webServer->url(), client->id());
    ...
    os_printf("ws[%s][%u] error(%u): %s\n", g_webServer->url(), client->id(), *((uint16_t*)arg), (char*)data);
  } else if(type == WS_EVT_PONG){
    //pong message was received (in response to a ping request maybe)
    os_printf("ws[%s][%u] pong[%u]: %s\n", g_webServer->url(), client->id(), len, (len)?(char*)data:"");
  } else if(type == WS_EVT_DATA){
    //data packet
    AwsFrameInfo * info = (AwsFrameInfo*)arg;
    if(info->final && info->index == 0 && info->len == len){
      //the whole message is in a single frame and we got all of it's data
      os_printf("ws[%s][%u] %s-message[%llu]: ", g_webServer->url(), client->id(), (info->opcode == WS_TEXT)?"text":"binary", info->len);
      ...
      if(info->index == 0){
        if(info->num == 0)
          os_printf("ws[%s][%u] %s-message start\n", g_webServer->url(), client->id(), (info->message_opcode == WS_TEXT)?"text":"binary");
        os_printf("ws[%s][%u] frame[%u] start[%llu]\n", g_webServer->url(), client->id(), info->num, info->len);
      }

      os_printf("ws[%s][%u] frame[%u] %s[%llu - %llu]: ", g_webServer->url(), client->id(), info->num, (info->message_opcode == WS_TEXT)?"text":"binary", info->index, info->index + len);
...
Browsers sometimes do not correctly close the websocket connection, even when the close() function is called in javascript.  This will eventually exhaust the web g_webServer's resources and will cause the g_webServer to crash.  Periodically calling the cleanClients() function from the main loop() function limits the number of clients by closing the oldest client when the maximum number of clients has been exceeded.  This can called be every cycle, however, if you wish to use less power, then calling as infrequently as once per second is sufficient.
...
The g_webServer includes EventSource (Server-Sent Events) plugin which can be used to send short text g_events to the browser.
Difference between EventSource and WebSockets is that EventSource is single direction, text-only protocol.

### Setup Event Source on the g_webServer
```cpp
AsyncWebServer g_webServer(80);
AsyncEventSource g_events("/g_events");
...
void setup(){
  ...
  g_events.setAuthentication("user", "pass");
...
void loop(){
  if(eventTriggered){ // your logic here
    //send event "myevent"
    g_events.send("my event content","myevent",millis());
  }
}
```
...
if (!!window.EventSource) {
  var source = new EventSource('/g_events');
...
g_webServer.on("/scan", HTTP_GET, [](AsyncWebServerRequest *request){
  String json = "[";
...
To remove handler:
```arduino
// sd_append callback for particular URL path
auto handler = g_webServer.on("/some/path", [](AsyncWebServerRequest *request){
  //do something useful
});
// when you don't need handler anymore remove it
g_webServer.removeHandler(&handler);

// same with rewrites
g_webServer.removeRewrite(&someRewrite);

g_webServer.onNotFound([](AsyncWebServerRequest *request){
  request->send(404);
});

// remove g_webServer.onNotFound handler
g_webServer.onNotFound(NULL);

// remove all rewrites, handlers and onNotFound/onFileUpload/onRequestBody callbacks
g_webServer.reset();
```

## Setting up the g_webServer
```cpp
...
AsyncEventSource g_events("/g_events"); // event source (Server-Sent g_events)
...
void setup(){
  ...
  ws.onEvent(onEvent);
  g_webServer.addHandler(&ws);

  // attach AsyncEventSource
  g_webServer.addHandler(&g_events);

  // respond to GET requests on URL /heap
  g_webServer.on("/heap", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", String(ESP.getFreeHeap()));
  });

  // upload a file to /upload
  g_webServer.on("/upload", HTTP_POST, [](AsyncWebServerRequest *request){
    request->send(200);
  }, onUpload);

  // send a file when /index is requested
  g_webServer.on("/index", HTTP_ANY, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/index.htm");
  });

  // HTTP basic authentication
  g_webServer.on("/login", HTTP_GET, [](AsyncWebServerRequest *request){
  ...
  g_webServer.on("/update", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/html", "<form method='POST' action='/update' enctype='multipart/form-data'><input type='file' name='update'><input type='submit' value='Update'></form>");
  });
  g_webServer.on("/update", HTTP_POST, [](AsyncWebServerRequest *request){
  ...
  g_webServer.serveStatic("/fs", SPIFFS, "/");

  // Catch-All Handlers
  // Any request that can not find a Handler that canHandle it
  // ends in the callbacks below.
  g_webServer.onNotFound(onRequest);
  g_webServer.onFileUpload(onUpload);
  g_webServer.onRequestBody(onBody);
...
void loop(){
  ...
  static char g_serialized[128];
  sprintf(g_serialized, "Seconds since boot: %u", millis()/1000);
  g_events.send(g_serialized, "time"); //send event "time"
}
```
...
```cpp
  g_webServer.on("^\\/sensor\\/([0-9]+)$", HTTP_GET, [] (AsyncWebServerRequest *request) {
