...
void FTPServer::begin(const String &uname, const String &pword)
{
  ...
  // Tells the ftp g_webServer to begin listening for incoming connections
  controlServer.begin();
  dataServer.begin();
}
...
void FTPServer::handleFTP()
{
  ...
  else if (cmdState == cWait) // FTP control g_webServer waiting for connection
  {
  ...
  else if (cmdState == cCheck) // FTP control g_webServer check/setup control connection
  {
    if (control.connected()) // A client connected, say "220 Hello client!"
    {
      FTP_DEBUG_MSG("control g_webServer got connection from %s:%d",
  ...
  else if (readChar() > 0)
  {
    ...
    // that should be supported to indicate g_webServer features even before login
...
// Read a char from client connected to ftp g_webServer
//
//  returns:
