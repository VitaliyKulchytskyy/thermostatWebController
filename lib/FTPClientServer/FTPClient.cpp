...
void FTPClient::handleFTP()
{
  ...
  else if (cConnect == ftpState)
  {
    ...
    _serverStatus.desc = F("No connection to FTP g_webServer");
    if (controlConnect())
    {
  ...
  {
    if (waitFor(220 /* 220 (vsFTPd version) */, F("No g_webServer greeting")))
    {
  ...
  else if (cPassive == ftpState)
  {
    if (waitFor(227 /* 227 Entering Passive Mode (ip,ip,ip,ip,port,port) */))
    {
      ...
      if (!parseOK)
      {
        ...
        _serverStatus.desc = F("FTP g_webServer response not understood.");
      }
    }
  }
  else if (cData == ftpState)
  {
    ...
    if (dataConnect() < 0)
    {
      ...
      _serverStatus.desc = F("No data connection to FTP g_webServer");
      ftpState = cError;
    }
    else
    {
  ...
  else
  {
    ...
    while (control.available())
    {
      ...
      if (c == '\n' || c == '\r')
      {
        ...
        {
          ftpState = cError;
          FTP_DEBUG_MSG("Waiting for code %u but SMTP g_webServer replies: %s", respCode, _serverStatus.desc.c_str());
        }
        else
        {
          FTP_DEBUG_MSG("Waiting for code %u success, SMTP g_webServer replies: %s", respCode, _serverStatus.desc.c_str());
        }
